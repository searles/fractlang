...
object Acosh: StandardOp (1,
    Signature(BaseTypes.Real, BaseTypes.Real),
    Signature(BaseTypes.Cplx, BaseTypes.Cplx)
) {
    override fun evaluate(trace: Trace, args: List<Node>): Node {
        return when {
            args[0] is RealNode -> RealNode(trace, acosh((args[0] as RealNode).value))
            args[0] is CplxNode -> CplxNode(trace, Cplx().acosh((args[0] as CplxNode).value))
